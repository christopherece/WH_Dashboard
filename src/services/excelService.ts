  ...
  async loadReversionTransitionDataFromOccupancy(): Promise<ReversionRecord[]> {
    ...
    return rawData.map((row) => {
      ...
      return {
        ...
        rawOccupancyStatus: this.parseNullableString(this.getFirstValue(normalizedRow, ['OccupancyStatus', 'ServiceStatus', 'sdStatus'])),
  ...
  private parseReversionData(rawData: any[]): ReversionRecord[] {
    return rawData.map((row) => {
      ...
      return {
        ...
        occupancyStatus,
        rawOccupancyStatus: rentalStatus || null,
        occupier,
        occupierType,
